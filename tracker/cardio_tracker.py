...
def analyze_cardio_session(session_data: CardioSession) -> Optional[CardioInsight]:
    """
    Uses Gemini to analyze the cardio session metrics and provide coaching.
    """
    ...
    - Avg Speed: {session_data.avg_speed_kmh} km/h
    - Calories: {session_data.calories_burned}
    - User Goal: {session_data.user_goal}
    ...
    try:
        response = client_gemini.models.generate_content(
            ...
            config=GenerateContentConfig(
                temperature=0.3,
                response_schema=CardioInsight, # Strict Output Structure
