...
BASE_DIR = "parquet/foods"

def foods_partition_path(date_str: str) -> str:
    d = datetime.fromisoformat(str(date_str))
    return f"{BASE_DIR}/year={d.year}/month={d.month:02d}/foods.parquet"
import pandas as pd

def upsert_foods_parquet(new_rows: list[dict]):
    if not new_rows:
        return
    ...
    for (year, month), df_part in df_new.groupby(
        [df_new["date"].dt.year, df_new["date"].dt.month]
    ):
        out_path = f"parquet/foods/year={year}/month={month:02d}/foods.parquet"
        os.makedirs(os.path.dirname(out_path), exist_ok=True)

        if os.path.exists(out_path):
            df_old = pd.read_parquet(out_path)
            df_old = df_old[~df_old["source_row_id"].isin(df_part["source_row_id"])]
            df_final = pd.concat([df_old, df_part], ignore_index=True)
        else:
            df_final = df_part
...
def etl_foods_only():
    ...
    for doc in diet_docs:
        food_rows.extend(flatten_diet_doc(doc))

    upsert_foods_parquet(food_rows)
    set_last_etl_run(datetime.utcnow())
